...
class AlexNet(torch.nn.Module):
    def __init__(self):
        super().__init__()
        # Prefer loading a local pretrained weight (offline-friendly).
        local_weight = "/share/home/202230550120/DiffSynth-Studio/评估/alexnet-owt-4df8aa71.pth"
        if os.path.exists(local_weight):
            m = models.alexnet(pretrained=False)
            state = torch.load(local_weight, map_location="cpu", weights_only=False)
            m.load_state_dict(state)
            self.model = m
        else:
            # fallback: let torchvision try to download if allowed
