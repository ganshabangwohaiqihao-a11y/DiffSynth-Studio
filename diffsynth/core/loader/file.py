...
def load_state_dict(file_path, torch_dtype=None, device="cpu", pin_memory=False, verbose=0):
    if isinstance(file_path, list):
        ...
        elif file_path.endswith(".index.json"):
            state_dict = load_state_dict_from_index_json(file_path, torch_dtype=torch_dtype, device=device)
        else:
...
def load_state_dict_from_index_json(file_path, torch_dtype=None, device="cpu"):
    """Load state dict from a .index.json file that maps keys to sharded weights."""
    with open(file_path, 'r') as f:
        weight_map = json.load(f)
    
    state_dict = {}
    weight_map_dict = weight_map.get("weight_map", {})
    
    # Get unique weight files
    weight_files = set(weight_map_dict.values())
    dir_path = os.path.dirname(file_path)
    
    # Load each weight file
    for weight_file in weight_files:
        weight_file_path = os.path.join(dir_path, weight_file)
        if weight_file_path.endswith(".safetensors"):
            partial_state = load_state_dict_from_safetensors(weight_file_path, torch_dtype=torch_dtype, device=device)
        else:
            partial_state = load_state_dict_from_bin(weight_file_path, torch_dtype=torch_dtype, device=device)
...
def load_keys_dict(file_path):
    ...
    if file_path.endswith(".safetensors"):
        ...
        return load_keys_dict_from_index_json(file_path)
    else:
...
def load_keys_dict_from_index_json(file_path):
    """Load keys dict from a .index.json file that maps keys to sharded weights."""
    with open(file_path, 'r') as f:
        weight_map = json.load(f)
    ...
    # Load keys from each weight file
    for weight_file in weight_files:
        weight_file_path = os.path.join(dir_path, weight_file)
        if weight_file_path.endswith(".safetensors"):
            partial_keys = load_keys_dict_from_safetensors(weight_file_path)
        else:
            partial_keys = load_keys_dict_from_bin(weight_file_path)
        keys_dict.update(partial_keys)
