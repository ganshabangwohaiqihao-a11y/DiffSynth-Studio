...
class SafetensorsCompatibleBinaryLoader:
    def __init__(self, path, device):
        ...
        self.state_dict = torch.load(path, weights_only=False, map_location=device)
...
class DiskMap:
    ...
    def __init__(self, path, device, torch_dtype=None, state_dict_converter=None, buffer_size=10**9):
        self.path = self.expand_paths(path)
        self.device = device
    ...
    def expand_paths(self, path):
        ...
        if path.endswith(".index.json"):
            ...
            ordered_weight_files = list(dict.fromkeys(weight_map.values()))
            return [os.path.join(base_dir, weight_file) for weight_file in ordered_weight_files]
        return [path]
