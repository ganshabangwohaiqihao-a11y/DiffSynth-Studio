...
def add_dataset_base_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--per_device_batch_size", type=int, default=1, help="Batch size per device (GPU).")
...
def add_training_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--lr_scheduler_type", type=str, default="constant", choices=["constant", "cosine", "linear"], help="Learning rate scheduler type. Options: constant (no decay), cosine (cosine annealing), linear (linear decay).")
    parser.add_argument("--lr_scheduler_kwargs", type=str, default="{}", help="Additional kwargs for scheduler in JSON format. E.g., for cosine: '{\"T_max\": 100, \"eta_min\": 0}'")
    return parser
...
def add_output_config(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--tensorboard_log_dir", type=str, default=None, help="TensorBoard log directory. If None, TensorBoard logging is disabled.")
    parser.add_argument("--tensorboard_log_interval", type=int, default=1, help="Log loss to TensorBoard every N training steps.")
    return parser
