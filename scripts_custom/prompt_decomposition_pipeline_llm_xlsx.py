#!/usr/bin/env python3
"""
Prompt Decomposition Pipeline (LLM-Powered) - XLSX Version
============================================================
Enhanced version using Qwen2-7B-Instruct for semantic room-type text extraction.
This version reads from XLSX files instead of CSV.
...
Usage:
  python prompt_decomposition_pipeline_llm_xlsx.py
...
# XLSX Input file path (replaces CSV_PATH)
XLSX_PATH = "/share/home/202230550120/diffusers/coperation.xlsx"
METADATA_PATH = "/share/home/202230550120/diffusers/output_add/chinese_meta_test.json"
OUTPUT_PATH = "/share/home/202230550120/diffusers/output_add/chinese_meta_test_enhanced_llm_xlsx.json"

# XLSX column names
PLAN_DESC_COLUMN = "户型描述"
PROMPT_COLUMN = "装修需求(布局要求)"
PLAN_ID_COLUMN = "plan_id"  # If exists; fallback to index
...
BATCH_SIZE = 32  # Process multiple prompts in parallel
MAX_RETRIES = 2
...
def has_relevant_content(prompt: str, room_type_id: Optional[int]) -> bool:
    ...
    Prevents fallback to global descriptions that don't match the room type.
    ...
    Returns:
        True if prompt contains relevant room keywords AND no strong contamination from other rooms
    """
    ...
    # 1) Check if extracted content contains at least one room-type keyword
    has_positive_match = any(cue in prompt for cue in cues)
    if not has_positive_match:
        return False
    
    # 2) Cross-room contamination check
    contamination_keywords = {
        "bedroom": ["主卧", "次卧", "卧室", "儿童房"],
        "dining_room": ["餐厅", "餐区", "用餐"],
        "living_room": ["客厅", "起居", "会客"],
    ...
    # Safe context rooms that can appear in other descriptions
    safe_context_rooms = {"bathroom", "living_dining_room"}
    
    # Check for contamination: if 2+ keywords from other rooms appear, reject
...
# ──────────────────────────────────────────────────────────────────────────────
# Main Pipeline - XLSX Version
# ──────────────────────────────────────────────────────────────────────────────

def load_prompt_mapping_from_xlsx() -> Dict[str, str]:
    """
    Load plan_id -> global_prompt mapping from XLSX file.
    
    Returns:
        Dictionary mapping plan_id to combined prompt (户型描述 + 装修需求)
    """
    try:
        import openpyxl
    except ImportError:
    ...
    print(f"\n📊 Available columns in XLSX: {list(df.columns)}")
    
    # Check if required columns exist
    has_plan_desc = PLAN_DESC_COLUMN in df.columns
    has_prompt = PROMPT_COLUMN in df.columns
    has_plan_id = PLAN_ID_COLUMN in df.columns
    
    print(f"   ✓ 户型描述: {has_plan_desc}")
    print(f"   ✓ 装修需求(布局要求): {has_prompt}")
    print(f"   ✓ 方案ID: {has_plan_id}")
    
    for idx, row in df.iterrows():
        # Use index as plan_id if column doesn't exist
        if has_plan_id:
            plan_id = str(int(row[PLAN_ID_COLUMN]))
        else:
            plan_id = str(idx)
        
        # Combine plan description and requirements
        plan_desc = str(row[PLAN_DESC_COLUMN]).strip() if has_plan_desc else ""
        prompt = str(row[PROMPT_COLUMN]).strip() if has_prompt else ""
        
        # Combine both columns
        combined_prompt = f"{plan_desc}。{prompt}" if plan_desc and prompt else (plan_desc or prompt)
        
        if combined_prompt:
            mapping[plan_id] = combined_prompt
...
def process_metadata(
    ...
    """
    Process metadata: extract room-specific prompts using LLM or rule-based approach.
    ...
    for idx, entry in enumerate(metadata):
        ...
        # Get global prompt from XLSX mapping
...
def main():
    print("=" * 70)
    print("PROMPT DECOMPOSITION PIPELINE (LLM-POWERED) - XLSX VERSION")
    print("=" * 70)
    ...
    # Load prompts from XLSX
    print("\n[2/4] Loading global prompts from XLSX...")
    prompt_mapping = load_prompt_mapping_from_xlsx()
    print(f"  ✓ Loaded {len(prompt_mapping)} global prompts from {XLSX_PATH}")
    ...
    print(f"Input source: {XLSX_PATH}")
