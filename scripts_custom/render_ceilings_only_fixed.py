# depth ≤ 1.0 视为基础面，不叠加 ceiling 颜色
# 1.0 到 20.0 -> 第1层
# 20.0 到 32.0 -> 第2层
# 32.0 到 45.0 -> 第3层
# 大于 45.0 -> 第4层
# 先按包含关系排：外层先画，内层后画
# 互不包含时按面积降序：大块先画，小块后画
# 第1层 -> 绿色
# 第2层 -> 紫色
# 第3层 -> 橙色
# 第4层 -> 蓝色
...
# Ceiling layer IDs for visualization/training (depth-binned layers + geometry ordering)
CEILING_LAYER_BASE_ID = 60
CEILING_LAYER_MAX = 8
CEILING_DEPTH_EPS = 1.0
CEILING_DEPTH_BIN_EDGES: Tuple[float, float, float] = (20.0, 32.0, 45.0)


def ceiling_layer_id(layer_idx: int) -> int:
    """Map ceiling layer index to semantic ID. Layer 0 -> first layer."""
    clipped = min(max(int(layer_idx), 0), CEILING_LAYER_MAX - 1)
    return CEILING_LAYER_BASE_ID + clipped


def ceiling_depth_to_layer_idx(depth_cm: float) -> Optional[int]:
    """Map raw depth (cm) to discrete layer index.

    Bins:
      depth <= 1.0          -> base plane (ignored for ceiling overlay)
      (1.0, 20.0]           -> layer 0
      (20.0, 32.0]          -> layer 1
      (32.0, 45.0]          -> layer 2
      (45.0, +inf)          -> layer 3
    """
    d = float(depth_cm)
    if d <= CEILING_DEPTH_EPS:
        return None
    e1, e2, e3 = CEILING_DEPTH_BIN_EDGES
    if d <= e1:
        return 0
    if d <= e2:
        return 1
...
def _sort_ceilings_within_bin(
    entries: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Sort one depth-bin group by containment first, area second.

    Parent (outer) polygons are rendered before children (inner) so inner details
    remain visible after overlay.
    """
    n = len(entries)
    if n <= 1:
        return entries
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            pj = entries[j]["poly"]
            aj = entries[j]["area"]
            if len(pj) < 3 or ai + 1e-6 < aj:
                continue
            cx, cy = _polygon_centroid(pj)
            if _point_in_polygon(cx, cy, pi):
                edges[i].append(j)
    ...
    # Kahn topological sort, with stable preference: larger area first.
    ready = [i for i in range(n) if indeg[i] == 0]
    ready.sort(key=lambda k: entries[k]["area"], reverse=True)
    ...
    if len(order) < n:
        ...
        remaining.sort(key=lambda k: entries[k]["area"], reverse=True)
...
def parse_floorplan(json_path: str) -> Dict[str, Any]:
    ...
    for r in data.get("roomData", []):
        ceilings: List[Dict] = []
        for c in r.get("ceilings", []):
            ceilings.append({
                "points": [(p["x"], p["y"]) for p in c.get("points", [])],
                "depth": float(c.get("depth", 0)),
            })
        rooms.append({
            "id": r["id"],
            "name": r.get("name", ""),
...
def _ensure_closed_polygon(pts: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
    """
    Ensure polygon is closed by adding first point to end if not already closed.
    This is critical for cv2.fillPoly to work correctly.
    """
    if len(pts) < 2:
        return pts

    # Convert to tuple format if needed (for dicts like {'x': ..., 'y': ...})
    def to_tuple(p):
    ...
    if abs(first[0] - last[0]) < 1e-6 and abs(first[1] - last[1]) < 1e-6:
        ...
        # If input is dict format, keep it; if tuple format, convert first point
        if isinstance(pts[-1], dict):
            return pts + [pts[0]]
        else:
            return pts + [first]
...
def render_room_no_ceil_and_with_ceil(
    room: Dict[str, Any],
    holes: Dict[str, Dict],
    target_size: Tuple[int, int] = (128, 128),
) -> Optional[Tuple[np.ndarray, np.ndarray, float, float, List[Tuple[float, float]], List[float], int]]:
    """Render two maps for one room:

    - no_ceil: floor + doors/windows only
    - with_ceil: floor + doors/windows + ceiling layers

        Ceiling rendering rule:
            1) bin by depth to discrete layers,
            2) within each bin, render by containment (outer before inner),
            3) non-contained peers are rendered by area (large before small).
    """
    pts = room["points"]
    if len(pts) < 3:
        return None
    ...
    no_ceil_canvas = np.zeros((canvas_h, canvas_w), dtype=np.uint8)

    def tx(pts_list: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        return [((p[0] - x_min) * scale, (p[1] - y_min) * scale) for p in pts_list]

    room_local = tx(pts)
    cv2.fillPoly(no_ceil_canvas, [_to_int32(room_local)], FLOOR_ID)

    room_mask = np.zeros_like(no_ceil_canvas, dtype=np.uint8)
    cv2.fillPoly(room_mask, [_to_int32(room_local)], 1)
    hole_mask = np.zeros_like(no_ceil_canvas, dtype=np.uint8)
    ...
    for hole in holes.values():
        ...
        if len(hole_pts) < 3:
            continue
        hole_local = tx(hole_pts)
        cat = HOLE_TYPE_MAP.get(hole.get("type", "门"), DOOR_ID)
        cv2.fillPoly(no_ceil_canvas, [_to_int32(hole_local)], cat)
        cv2.fillPoly(hole_mask, [_to_int32(hole_local)], 1)

    with_ceil_canvas = no_ceil_canvas.copy()

    bins: Dict[int, List[Dict[str, Any]]] = defaultdict(list)
    for ceiling in room.get("ceilings", []):
        c_pts = ceiling.get("points", [])
        if len(c_pts) < 3:
            continue
        # Ensure polygon is closed for fillPoly to work correctly
        c_pts_closed = _ensure_closed_polygon(c_pts)
        local_poly = tx(c_pts_closed)
        poly_i32 = _to_int32(local_poly)

        layer_idx = ceiling_depth_to_layer_idx(float(ceiling.get("depth", 0)))
        if layer_idx is None:
            # Base plane ceiling (depth <= eps) is treated as floor plane.
            continue

        bins[layer_idx].append(
            {
                "poly": local_poly,
                "poly_i32": poly_i32,
                "area": _polygon_area(local_poly),
            }
        )

    # Ceiling overlays are only valid inside room interior and should not overwrite doors/windows.
    valid_ceiling_mask = (room_mask == 1) & (hole_mask == 0)

    rendered_layer_ids = set()
    for layer_idx in sorted(bins.keys()):
        ordered = _sort_ceilings_within_bin(bins[layer_idx])
        for item in ordered:
            obj_mask = np.zeros_like(with_ceil_canvas, dtype=np.uint8)
            cv2.fillPoly(obj_mask, [item["poly_i32"]], 1)
            # Keep first painted ceiling layer on each pixel to avoid unstable
            # cross-layer overwrite caused by inconsistent depth annotations.
            write_mask = (obj_mask == 1) & valid_ceiling_mask & (with_ceil_canvas == FLOOR_ID)
            with_ceil_canvas[write_mask] = ceiling_layer_id(layer_idx)
        rendered_layer_ids.add(layer_idx)

    rendered_layers = len(rendered_layer_ids)
    ...
    no_ceil_padded = np.pad(
        no_ceil_canvas,
        ((pad_top, pad_bottom), (pad_left, pad_right)),
        mode="constant",
        constant_values=VOID_ID,
    )
    with_ceil_padded = np.pad(
        with_ceil_canvas,
        ((pad_top, pad_bottom), (pad_left, pad_right)),
        mode="constant",
        constant_values=VOID_ID,
...
_VIZ_COLORMAP: Dict[int, Tuple[int, int, int]] = {
    ...
    DOOR_ID:   (255, 120,  0),
    WINDOW_ID: (0,  180, 255),
    CEILING_LAYER_BASE_ID + 0: (70, 170, 70),      # 第1层：绿色 (Green)
    CEILING_LAYER_BASE_ID + 1: (175, 90, 215),     # 第2层：紫色 (Purple)
    CEILING_LAYER_BASE_ID + 2: (235, 170, 60),     # 第3层：橙色 (Orange)
    CEILING_LAYER_BASE_ID + 3: (70, 160, 210),     # 第4层：蓝色 (Blue)
    CEILING_LAYER_BASE_ID + 4: (210, 95, 95),      # 第5层：红色 (Red)
    CEILING_LAYER_BASE_ID + 5: (120, 120, 220),    # 第6层：浅蓝 (Light Blue)
    CEILING_LAYER_BASE_ID + 6: (80, 180, 170),     # 第7层：青色 (Cyan)
    CEILING_LAYER_BASE_ID + 7: (190, 145, 80),     # 第8层：棕色 (Brown)
...
def process_all(
    ...
    collected: Dict[str, dict] = {s: {"layout_with_ceiling": [], "layout_no_ceil": [], "global": [], "meta": []}
    ...
    for path in tqdm(json_paths, desc="Pass 2 – rendering"):
        ...
        for room in fp_data["rooms"]:
            ...
            result = render_room_no_ceil_and_with_ceil(
                room=room,
                holes=fp_data["holes"],
                target_size=(SH, SW),
            )
            ...
            sample_with_ceiling = np.stack([room_type_layer, sem_map_with_ceiling], axis=0)
            sample_no_ceil = np.stack([room_type_layer, sem_map_no_ceil], axis=0)

            bucket = collected[split]
            bucket["layout_with_ceiling"].append(sample_with_ceiling)
            bucket["layout_no_ceil"].append(sample_no_ceil)
            ...
            bucket["meta"].append({
                ...
                "n_ceilings":   len(room.get("ceilings", [])),
                "n_ceiling_layers_rendered": n_ceiling_layers,
                "room_width_cm":  float(room_width_cm),
                "room_height_cm": float(room_height_cm),
            })
            total_rooms += 1

            # Optional visualisation (no_ceil and with_ceiling)
            if visualize and viz_count < max_viz:
                if show_dimensions:
                    viz_path_with = str(viz_dir / f"{plan_id}_{room['id']}_{room_name}_with_ceiling.png")
                    ...
                    viz_path_no_ceil = str(viz_dir / f"{plan_id}_{room['id']}_{room_name}_no_ceil.png")
                    save_visualization_with_dimensions(
                        sem_map_no_ceil,
                        room_width_cm,
                        room_height_cm,
                    ...
                    viz_path_with = str(viz_dir / f"{plan_id}_{room['id']}_{room_name}_with_ceiling.png")
                    save_visualization(sem_map_with_ceiling, viz_path_with)

                    viz_path_no_ceil = str(viz_dir / f"{plan_id}_{room['id']}_{room_name}_no_ceil.png")
                    save_visualization(sem_map_no_ceil, viz_path_no_ceil)
    ...
    for split_name, bucket in collected.items():
        samples = bucket["layout_with_ceiling"]
        samples_no_ceil = bucket["layout_no_ceil"]
        if not samples:
            print(f"  [{split_name}] No samples — skipping.")
            continue

        layout_arr = np.stack(samples, axis=0)             # (N, 2, H, W) uint8 - with ceiling
        layout_no_ceil_arr = np.stack(samples_no_ceil, axis=0)  # (N, 2, H, W) uint8 - no ceiling
        global_arr = np.array(bucket["global"], dtype=np.uint8)  # (N, GH, GW)

        n_s = len(samples)
        out_layout = os.path.join(output_dir, f"chinese_with_ceiling_{split_name}_{SH}x{SW}.npy")
        out_layout_no_ceil = os.path.join(output_dir, f"chinese_no_ceil_{split_name}_{SH}x{SW}.npy")
        ...
        np.save(out_layout_no_ceil, layout_no_ceil_arr)
        ...
        with open(out_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(
                ...
                fieldnames=["plan_id", "room_id", "room_name", "room_type_id",
                             "room_type", "n_ceilings", "n_ceiling_layers_rendered",
        ...
        pct_ceil = (sem_maps >= CEILING_LAYER_BASE_ID).any(axis=(1, 2)).mean() * 100

        print(f"\n[{split_name}]  {n_s} rooms")
        print(f"  Layout (with ceiling) → {out_layout}  shape={layout_arr.shape}")
        print(f"  Layout (no_ceil)      → {out_layout_no_ceil}  shape={layout_no_ceil_arr.shape}")
        ...
        print(f"  Rooms with any ceiling layer: {pct_ceil:.1f}%")
...
def main():
    parser = argparse.ArgumentParser(
        description="Render room floor/door/window + ceiling-layer maps (no_ceil and with_ceiling)",
